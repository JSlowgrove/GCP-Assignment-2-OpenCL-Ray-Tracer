		...
		for (unsigned int x = 0; x < xLoopNum; x++)
		{
			...
			for (unsigned int box = 0; box < numberOfBoxes; box++)
			{
				...
				for (int i = 0; i < 12; i++)
				{
					...
					if (rayTriangleIntersect(rayOrigin, rayDirection, vertices[index],
...
std::vector<unsigned char> RayTracer::rayTraceBoxes(std::vector<Box> boxes, glm::vec2 screenDim, glm::vec4 rayDirection, bool useOpenCL)
{
	...
	for (unsigned int box = 0; box < boxes.size(); box++)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			vertices.push_back(boxes[box].getTriangleVerticies()[index]);
			vertices.push_back(boxes[box].getTriangleVerticies()[index + 1]);
